...
export function PhotoGallery({ activityId, photos }: { activityId: string; photos: ActivityPhoto[] }) {
  if (!photos.length) return <p className="rounded-2xl border bg-white/70 p-5 text-sm text-muted-foreground">Nessuna foto caricata per questo evento.</p>;
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {photos.map((photo) => (
        <figure key={photo.id} className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          {photo.signedUrl ? (
            <Image src={photo.signedUrl} alt={photo.caption || "Foto evento"} width={800} height={600} className="aspect-[4/3] w-full object-cover" />
          ) : (
            <div className="aspect-[4/3] bg-muted" />
          )}
          <figcaption className="flex items-center justify-between gap-3 p-3 text-sm text-muted-foreground">
            <span>{photo.caption || "Foto evento"}</span>
